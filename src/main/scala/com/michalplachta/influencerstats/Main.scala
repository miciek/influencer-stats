...
import com.michalplachta.influencerstats.api.{youtube, HttpRoutes}
...
object Main extends App {
  ...
  def getInfluencerResults(id: UUID): Future[InfluencerResults] = {
    ...
    Http()
      ...
      .map(_.items.map(_.statistics).map(video => InfluencerItem(video.viewCount, video.likeCount, 0, 0)))
      .map(Statistics.calculate)
  }

  val httpApp = new HttpApp {
    override protected def routes: Route =
      Route.seal(HttpRoutes.getInfluencerResults(getInfluencerResults))
  }
